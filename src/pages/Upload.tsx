...
import {
  UploadCloud, FileSpreadsheet, CheckCircle,
...
// ─── Heuristic-only fallback — scans ALL rows ─────────────────────────────────
...
function runHeuristicAnalysis(
  ...
  // 1. Duplicate detection — full scan O(n)
  ...
  rows.forEach((row, idx) => {
    ...
    if (urlFirstSeen.has(urlVal)) {
      anomalies.push({
        row: idx + 1,
        reason: `Duplicate URL: ${urlVal.slice(0, 80)} (first at row ${urlFirstSeen.get(urlVal)! + 1})`,
        severity: 'medium',
      });
      flaggedRows.add(idx);
    } else {
      urlFirstSeen.set(urlVal, idx);
    }
  });

  // 2. Missing values — full scan
  rows.forEach((row, idx) => {
    const missing = Object.values(row).filter(v => !v?.trim()).length;
    if (missing > headers.length * 0.5) {
      anomalies.push({
        row: idx + 1,
        reason: `${missing}/${headers.length} fields are empty`,
        severity: 'low',
      });
      flaggedRows.add(idx);
    }
  ...
  // 3. URL pattern checks — full scan
  rows.forEach((row, idx) => {
    ...
    if (kwHits.length >= 2) {
      anomalies.push({
        row: idx + 1,
        reason: `Phishing keywords: "${kwHits.join('", "')}"`,
        severity: 'high',
      });
      flaggedRows.add(idx);
    }
    ...
    if (IP_RE.test(urlVal)) {
      anomalies.push({
        row: idx + 1,
        reason: 'URL uses raw IP address instead of domain name',
        severity: 'high',
      });
      flaggedRows.add(idx);
    }
    ...
    if (SHORTENERS.some(s => urlVal.includes(s))) {
      anomalies.push({
        row: idx + 1,
        reason: 'URL shortener detected — true destination is hidden',
        severity: 'medium',
      });
      flaggedRows.add(idx);
    }
  ...
  // Keep highest-severity anomaly per row, sort by row number
  ...
  return {
    ...
    cleanRows,
    suspiciousRows,
    anomalies: dedupedAnomalies.slice(0, 30),
    summary:
      suspiciousRows === 0
        ? `✅ All ${totalRows} rows passed heuristic analysis. No anomalies detected. (Note: Flask backend is offline — ML model predictions unavailable.)`
        : `⚠️ Backend connection error — heuristic analysis only. ${suspiciousRows} potentially suspicious rows detected. Ensure Flask backend is running for ML model predictions.`,
    score: cleanScore,
    preCheckStats,
...
async function analyzeWithBackend(
  ...
  (br.results || []).forEach((r: any, idx: number) => {
    if (r.error) {
      ...
      anomalies.push({
        row: idx + 1,
        reason: `Phishing detected — ${r.confidence}% confidence${reasons}`,
        severity: sev,
      });
    }
  });
  ...
  return {
    ...
    score: cleanScore,
    preCheckStats,
    detectionMethod: br.detection_method,
...
function SeverityBadge({ s }: { s: 'high' | 'medium' | 'low' }) {
  ...
  return (
    <span className={`text-[10px] px-1.5 py-0.5 rounded border font-mono uppercase flex-shrink-0 ${map[s]}`}>
      {s}
    </span>
  );
}
      ...
      {flags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {flags.map(f => (
            <span
              key={f.label}
              className="flex items-center gap-1 text-[10px] bg-white/5 border border-white/10 px-2 py-1 rounded-lg"
            >
...
export function Upload() {
  ...
  const processFile = async (f: File) => {
    ...
    try {
      ...
      const urlCol = headers.find(
        h => h.toLowerCase().includes('url') || h.toLowerCase() === 'link'
      );
      const urls = urlCol ? rows.map(r => r[urlCol]).filter(Boolean) : [];

      // ── Pre-checks phase (sample up to 100 URLs) ─────────────────────────
      setUploadState('prechecking');

      const preCheckStats: PreCheckStats = {
        total: urls.length,
        safe: 0, low: 0, medium: 0, high: 0, critical: 0,
        withPaywall: 0, withRedirect: 0, withShortener: 0,
        withFirewallBypass: 0, fromCache: 0,
      };
      ...
      if (urls.length > 0) {
        ...
        for (let i = 0; i < sample.length; i += BATCH) {
          ...
          // Sequential — no concurrent mutations
      ...
      try {
        ...
        // Backend down — run full heuristic scan on ALL rows
  ...
  const scoreColor =
    (result?.score ?? 0) >= 80 ? 'text-neon-green' :
    (result?.score ?? 0) >= 50 ? 'text-yellow-300' : 'text-neon-red';
      ...
      <AnimatePresence mode="wait">
        ...
        {uploadState === 'done' && result && (
          <motion.div key="done" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            {result.preCheckStats && <PreCheckSummaryCard stats={result.preCheckStats} />}
            ...
            {result.anomalies.length > 0 ? (
              <div className="glass-panel p-6">
                <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
                  ...
                  Flagged Anomalies ({result.anomalies.length}{result.anomalies.length === 30 ? '+' : ''})
                </h3>
                ...
                {result.anomalies.length > 5 && (
                  ...
                  >
                    {showAll
                      ...
                      : <><ChevronDown size={12} /> Show all {result.anomalies.length}</>
                    }
                  </button>
                )}
              </div>
              ...
              <div className="glass-panel p-6 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Dataset appears clean. No phishing URLs or duplicates found.</p>
                </div>
              </div>
            )}
